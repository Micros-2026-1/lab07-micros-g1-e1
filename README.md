...
I²C usa solo dos líneas (SDA y SCL) compartidas para enviar y recibir, por lo que en un momento dado el bus va en un solo sentido: o el maestro transmite o el esclavo responde, nunca los dos al mismo tiempo → half-duplex. SPI tiene líneas separadas para MOSI y MISO, permitiendo transmisión simultánea en ambas direcciones full-duplex.Para una LCD la implicación es mínima en la práctica, porque la LCD casi nunca le devuelve datos al PIC (solo se escribe). El half-duplex no representa una pérdida real aquí.
...
El valor 1000 en SSPM es el que selecciona específicamente el modo I²C maestro. Se elige este porque el PIC es quien controla el bus: genera el clock, inicia y termina las transmisiones.
...
SSPIF (SSP Interrupt Flag) es una bandera de hardware que el módulo MSSP levanta automáticamente cuando termina una operación: condición de start, stop, o transmisión de un byte completo. El while(!SSPIF) es una espera activa que bloquea el código hasta que esa operación termine.
Se limpia manualmente (PIR1bits.SSPIF = 0) porque el hardware la levanta pero no la baja solo. Si no se limpia, la siguiente espera while(!SSPIF) retornaría inmediatamente sin que haya terminado la operación real, corrompiendo la comunicación.
...
En modo paralelo se usan entre 6 y 8 pines del PIC directamente conectados a la LCD (RS, EN y los datos), lo que consume una parte importante del puerto pero el envío es directo y rápido. En modo I²C solo se usan 2 pines (SCL y SDA) gracias al expansor PCF8574 que hace la conversión, pero cada byte pasa por el protocolo I²C y el empaquetado de nibbles, lo que añade retardo.
En cuanto a complejidad, el modo paralelo es más sencillo de entender porque cada pin tiene una función clara y visible. El modo I²C requiere manejar la capa del protocolo, las condiciones de start y stop, la dirección del esclavo y la lógica de nibbles, lo que resulta en más código y más posibilidades de fallo. Sin embargo, en proyectos reales donde los pines son un recurso limitado, ese costo en complejidad y velocidad generalmente vale la pena.
...
En hardware el cambio es simplemente conectar el segundo módulo a las mismas líneas SDA y SCL, y configurar sus pines A0–A2 con una combinación diferente a la del primero para que tenga una dirección I²C distinta (por ejemplo, el primero en 0x27 y el segundo en 0x26).
En el código el cambio mínimo es definir una segunda constante de dirección y usarla en I2C_write() al inicio de cada transacción, en lugar de ADDRESS_LCD. Con eso el maestro sabe a cuál de los dos módulos le está hablando en cada momento.

## Conclusiones
1. La implementación en modo I²C demuestra que es posible controlar una LCD HD44780 con solo dos pines del microcontrolador, lo que representa una ventaja significativa en diseños donde los recursos de E/S son limitados. El uso del expansor PCF8574 como intermediario permite mantener la compatibilidad con el protocolo paralelo nativo de la LCD sin sacrificar pines del PIC.
2. El correcto funcionamiento del sistema depende de una cadena de abstracción bien estructurada: la configuración precisa del módulo MSSP (modo maestro, baudrate de 100 kHz, slew rate controlado) es la base sobre la que se construyen las funciones de start, stop y write, y estas a su vez sostienen toda la lógica del LCD. Un error en cualquier nivel de esa cadena afecta todo lo que está encima.
3. La capacidad del bus I²C de soportar múltiples esclavos sobre las mismas dos líneas, diferenciados únicamente por su dirección, lo convierte en una solución escalable para sistemas embebidos complejos. En este laboratorio se usa un solo LCD, pero el diseño del código permite agregar un segundo módulo con un cambio mínimo, lo que demuestra que una buena arquitectura de comunicación facilita la expansión del sistema sin rediseñar el hardware.
## Referencias